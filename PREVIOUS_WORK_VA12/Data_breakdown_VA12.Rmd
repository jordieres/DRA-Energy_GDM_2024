...
formas = list(coil = coil, sheet = sheet, strip = strip)
...
```
```{r}
variables <- c("Componente2kW/Num_productos","Componente7kW/Num_productos")
umbrales <- c(3000, 2000)
...
```{r}
dat <- subset(datos, (Componente2kW/Num_productos) < 3000 &  (Componente7kW/Num_productos) < 2000)
...
# Crear una lista con los data frames filtrados
formas2 = list(coil2 = coil2, sheet2 = sheet2, strip2 = strip2)

cons <- names(dat)[c(28,33)]
...
corrplot(cor(dat[,c(4,5,7:33)]), method = 'ellipse')
...
b = dat$Componente1kW+dat$Componente2kW+dat$Componente3kW+dat$Componente4kW+dat$Componente5kW+dat$Componente6kW+dat$Componente7kW
for(i in vars){
    a = (dat[[i]]/dat$Num_productos)
...
plot(b~a, main= "Tiempo de producción vs Longitud", ylab = "Tiempo de producción(min)", xlab = "Longitud")
plot(datos$Velocidad, main="Velocidad", xlab = "Observacion", ylab = "m/min")
```

```{r}
mean(datos$Longitud_m/datos$Tiempo_produccion)
mean(datos$Velocidad)
```
...
densidad = density(datos$Velocidad)
