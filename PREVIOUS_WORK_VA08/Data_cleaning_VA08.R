...
datos <- read_excel("linea_VA08.guay.xlsx")
...
secuencias = contar_repeticiones(datos2$ProductionOrderNR)
datos2$secuencias = secuencias


resultado2 <- datos2 %>%
  group_by(ProductionOrderNR) %>%
  summarise(parametroA = ProductionOrderParameterA,
            parametroB = ProductionOrderParameterB,
            parametroC = ProductionOrderParameterC,
            parametroD = ProductionOrderParameterD,
            parametroE = ProductionOrderParameterE,
            parametroF = ProductionOrderParameterF,
            parametroG = ProductionOrderParameterG,
            parametroHMin = ProductionOrderParameterHMin,
            parametroHMax = ProductionOrderParameterHMax,
            parametroI = ProductionOrderParameterI,
            ...
            Componente5kW= sum(Component5, na.rm = TRUE),
            Componente6kW= sum(Component6, na.rm = TRUE),
...
resultado3 <- merge(res, res2, by.x = "order no.", by.y = "ProductionOrderNR")
...
resultado4 <- resultado3 %>%
  ...
  summarise(
    ...
    Componente5kW= Componente5kW,
    Componente6kW= Componente6kW,
    Recocido = Recocido,
    Reduccion = Reduccion,
...
VA08_final_final_final = "VA08_final_final_final.xlsx"

write.xlsx(res_unique, file = VA08_final_final_final, rowNames = FALSE)
