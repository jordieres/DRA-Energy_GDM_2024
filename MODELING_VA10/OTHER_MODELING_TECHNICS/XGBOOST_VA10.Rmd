...
nuevos_datos <- datos %>%
  select(8, 9, 17:26, 34:40) %>%
  filter(Componente4kW_unidad < 2500)
...
train_data <- dat$train_df %>% select(-c(Componente1kW_unidad, Componente2kW_unidad, Componente3kW_unidad, Componente4kW_unidad))
test_data <- dat$test_df %>% select(-c(Componente1kW_unidad, Componente2kW_unidad, Componente3kW_unidad, Componente4kW_unidad))

# Crear matrices de entrenamiento y prueba para cada componente
dat$train_mats <- lapply(1:4, function(i) {
  xgb.DMatrix(data = as.matrix(train_data), label = dat$train_df[[paste0("Componente", i, "kW_unidad")]])
})

dat$test_mats <- lapply(1:4, function(i) {
...
errores_df <- do.call(rbind, lapply(1:4, function(i) {
  data.frame(
